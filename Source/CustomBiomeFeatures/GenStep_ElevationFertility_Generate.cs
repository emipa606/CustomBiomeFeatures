...
{
    [HarmonyPatch(typeof(GenStep_ElevationFertility), "Generate", typeof(Map), typeof(GenStepParams))]
    [HarmonyPriority(100)]
    public class GenStep_ElevationFertility_Generate
    {
        private static bool Prefix(ref Map map)
        {
            NoiseRenderer.renderSize = new IntVec2(map.Size.x, map.Size.z);
            ModuleBase input = new Perlin(0.020999999716877937, 2.0, 0.5, 6, Rand.Range(0, int.MaxValue),
                QualityMode.High);
            ...
            {
                if (settings.allowedHilliness.All(x => !x.Value))
                {
                    b = 0;
                }
            }
            ...
            ModuleBase input3 = new Perlin(0.020999999716877937, 2.0, 0.5, 6, Rand.Range(0, int.MaxValue),
                QualityMode.High);
